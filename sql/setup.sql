...
-- ============================================================
-- STEP 2.5: Compute Pool の作成
--   Notebook 実行環境として使用
-- ============================================================
CREATE COMPUTE POOL IF NOT EXISTS COSME_EC_COMPUTE_POOL
    MIN_NODES = 1
    MAX_NODES = 1
    INSTANCE_FAMILY = CPU_X64_S
    AUTO_SUSPEND_SECS = 300
    AUTO_RESUME = TRUE
    COMMENT = 'コスメECハンズオン用 Compute Pool（Notebook 実行環境）';
...
COPY FILES INTO @COSME_EC_HANDSON.RAW.HANDSON_RESOURCES/eval/
    FROM @COSME_EC_HANDSON.RAW.GIT_COSME_EC_HANDSON/branches/main/eval/
    PATTERN = '.*\.json$';

-- コピーされたファイルの確認
LIST @COSME_EC_HANDSON.RAW.HANDSON_RESOURCES;

-- ============================================================
-- STEP 9: ファイルフォーマットの作成
-- ============================================================
...
CREATE OR REPLACE FILE FORMAT COSME_EC_HANDSON.RAW.JSON_FORMAT
    TYPE = 'JSON'
    STRIP_OUTER_ARRAY = TRUE
    COMMENT = 'JSON取り込み用フォーマット（外側配列をストリップ）';
...
-- ============================================================
-- STEP 18: エージェント評価用の権限設定
--   Cortex Agent Evaluations に必要なロール・権限を付与
--   参考: https://www.snowflake.com/en/developers/guides/getting-started-with-cortex-agent-evaluations/
-- ============================================================
USE ROLE ACCOUNTADMIN;
...
GRANT CREATE DATASET ON SCHEMA COSME_EC_HANDSON.ANALYTICS TO ROLE ACCOUNTADMIN;
GRANT CREATE TASK ON SCHEMA COSME_EC_HANDSON.ANALYTICS TO ROLE ACCOUNTADMIN;
GRANT EXECUTE TASK ON ACCOUNT TO ROLE ACCOUNTADMIN;

-- ============================================================
-- STEP 19: 評価テーブル（EVALS_TABLE）の作成とデータ投入
--   Ground Truth データ（csv/evals.json）を JSON 形式でロード
--   各行は INPUT_QUERY（質問文）と GROUND_TRUTH_DATA（期待ツール・期待回答）で構成
-- ============================================================
USE SCHEMA ANALYTICS;

CREATE OR REPLACE TABLE COSME_EC_HANDSON.ANALYTICS.EVALS_TABLE (
    INPUT_QUERY TEXT       COMMENT '評価用の質問文',
    GROUND_TRUTH_DATA VARIANT  COMMENT '期待されるツール呼び出しと回答（JSON）'
) COMMENT = 'エージェント評価用 Ground Truth データ（21件: FAQ 4件、商品検索 2件、注文分析 6件、レビュー分析 4件、複合 5件）';

COPY INTO COSME_EC_HANDSON.ANALYTICS.EVALS_TABLE
FROM (
    SELECT
        $1:INPUT_QUERY::TEXT,
        $1:GROUND_TRUTH_DATA::VARIANT
    FROM @COSME_EC_HANDSON.RAW.HANDSON_RESOURCES/eval/evals.json
)
FILE_FORMAT = (FORMAT_NAME = 'COSME_EC_HANDSON.RAW.JSON_FORMAT')
ON_ERROR = 'CONTINUE';

-- ============================================================
-- STEP 20: 評価設定ステージの作成 + YAML設定ファイルのアップロード
-- ============================================================
CREATE OR REPLACE STAGE COSME_EC_HANDSON.ANALYTICS.EVAL_CONFIG_STAGE
    DIRECTORY = (ENABLE = TRUE)
    COMMENT = 'エージェント評価設定ファイル格納用ステージ';

COPY FILES INTO @COSME_EC_HANDSON.ANALYTICS.EVAL_CONFIG_STAGE
    FROM @COSME_EC_HANDSON.RAW.GIT_COSME_EC_HANDSON/branches/main/eval/
    FILES = ('cosme_agent_eval_config.yaml');

LS @COSME_EC_HANDSON.ANALYTICS.EVAL_CONFIG_STAGE;

-- ============================================================
-- STEP 21: データ確認サマリ（評価テーブル）
-- ============================================================
SELECT 'EVALS_TABLE' AS "テーブル名", COUNT(*) AS "件数" FROM COSME_EC_HANDSON.ANALYTICS.EVALS_TABLE;
